...
import Button from "../UI/Button/Button";import { addNEwItemAction} from "../store/basketReducer";
import { decrAction, incrAction } from "../store/productInfoReducer";
...
function ProductIndoPage() {
  ...
  const {productInfo, count} = useSelector((store) => store.productInfo);
  ...
  const handleButtonClick = (event, elem) => {
    ...
    dispatch(addNEwItemAction({...elem, count}));
  }
